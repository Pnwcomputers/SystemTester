...
# Complete Production Version - v3
...
# Self-identity (v3): used by the event-log threat audit to recognize and
# discount its OWN execution noise (its 4104 script blocks, and Defender
# detections that fire on this script's content) so real threats aren't buried.
$script:SelfScriptPath = if ($PSCommandPath) { $PSCommandPath }
                         elseif ($MyInvocation.MyCommand.Path) { $MyInvocation.MyCommand.Path }
                         else { "" }
$script:SelfScriptName = if ($script:SelfScriptPath) { [System.IO.Path]::GetFileName($script:SelfScriptPath) } else { "SystemTester.ps1" }
...
function Test-EventLogThreats {
    ...
    # v3: separate detections that point at THIS diagnostic tool (its own
    # script content trips generic "PSAttackTool"-style AMSI signatures) from
    # genuine external detections. Self-attributed items are still listed and
    # annotated, but do NOT drive the CRITICAL "possible malware" escalation.
    $defExternal = 0
    $defSelf = 0
    $defLines = @()
    foreach ($ev in ($defDetect | Select-Object -First 15)) {
        ...
        $dpath = ""
        if ($ev.Message -match "Path:\s*([^\r\n]+)") { $dpath = $matches[1].Trim() }
        $isSelf = $false
        if ($dpath) {
            if ($script:SelfScriptPath -and $dpath -match [regex]::Escape($script:SelfScriptPath)) { $isSelf = $true }
            elseif ($dpath -match [regex]::Escape($script:SelfScriptName)) { $isSelf = $true }
        }
        $line = "  [{0:yyyy-MM-dd HH:mm}] ID {1}: {2}" -f $ev.TimeCreated, $ev.Id, $threat
        if ($dpath) { $line += " | Path: $dpath" }
        if ($isSelf) { $line += "  (attributed to THIS diagnostic tool - verify path)"; $defSelf++ }
        else { $defExternal++ }
        $defLines += $line
    }
    # Unclassified overflow beyond the first 15 is counted as external (conservative)
    if ($defDetect.Count -gt 15) { $defExternal += ($defDetect.Count - 15) }
    $out += "EventLog-DefenderDetections: $defExternal"
    $out += "EventLog-DefenderDetectionsSelf: $defSelf"
    if ($defDetect.Count -gt 0) {
        $out += ">>> DEFENDER MALWARE DETECTIONS (30 days):"
        $defLines | ForEach-Object { $out += $_ }
        if ($defDetect.Count -gt 15) { $out += "  ... and $($defDetect.Count - 15) more" }
        if ($defSelf -gt 0) {
            $out += "  Note: $defSelf detection(s) point at this diagnostic tool's own script"
            $out += "        (generic PowerShell-tool signature) - not counted as external malware"
        }
    }
    ...
    # v3: exclude script blocks generated by THIS tool while it runs. Without
    # this the tester floods its own report with its own 4104 events (the cert
    # callback, download helpers, etc. are Warning-level under AMSI). Match by
    # the 4104 Path property first, then fall back to highly specific content
    # markers for blocks that carry no path.
    $selfBlockMarkers = @(
        'Pacific Northwest Computers','PNWC System Tester','Test-MalwareScan',
        'Test-EventLogThreats','Invoke-SysinternalsCsv','Get-VTDetection','$script:TestResults'
    )
    $psSusp = @()
    $psSelfCount = 0
    foreach ($ev in $psEvents) {
        if (-not ($ev.LevelDisplayName -eq "Warning" -or $ev.Message -match $psKeywordRegex)) { continue }
        $isSelf = $false
        $blkPath = ""
        try { if ($ev.Properties.Count -ge 5) { $blkPath = "$($ev.Properties[4].Value)" } } catch {}
        if ($blkPath) {
            if ($script:SelfScriptPath -and $blkPath -eq $script:SelfScriptPath) { $isSelf = $true }
            elseif ([System.IO.Path]::GetFileName($blkPath) -ieq $script:SelfScriptName) { $isSelf = $true }
        }
        if (-not $isSelf) {
            $blkText = ""
            try { if ($ev.Properties.Count -ge 3) { $blkText = "$($ev.Properties[2].Value)" } } catch {}
            foreach ($m in $selfBlockMarkers) { if ($blkText -like "*$m*") { $isSelf = $true; break } }
        }
        if ($isSelf) { $psSelfCount++ } else { $psSusp += $ev }
    }
    $psSusp = @($psSusp)

    $out += "EventLog-SuspiciousPowerShell: $($psSusp.Count)"
    if ($psSelfCount -gt 0) {
        $out += "  (excluded $psSelfCount script block(s) generated by this diagnostic tool itself)"
    }
    if ($psSusp.Count -gt 0) {
        ...
        $out += "  Note: admin tools (RMM, installers) can also trigger these - verify context"
    }
...
function New-Report {
    ...
    if ($malAutoruns -or $malProcs -or $malEvents) {
        ...
        if ($malEvents) {
            ...
            if ($malEvents.Output -match "EventLog-DefenderDetectionsSelf: (\d+)") { $defSelfDetections = [int]$matches[1] }
        ...
        # v3: context note so a tech isn't alarmed by detections that are just
        # this tool's own content matching a generic PowerShell-tool signature.
        if ($defSelfDetections -gt 0) {
            $recommendations += "* INFO: $defSelfDetections Defender detection(s) matched THIS diagnostic tool's own script"
            $recommendations += "  -> Expected - the tool contains threat-hunting keywords AV may flag (see AMSI notes)"
            $recommendations += "  -> Not counted as an external threat; verify the flagged Path if unsure"
        }
    }
